...
class Abstract_Graph_Summary(object):
    ...
    def get_merge_candidates(self,supernode_name):
        return self.exact_two_hop_neighbors(supernode_name,2)
    ...
    def exact_n_hop_neighbors(self,supernode_name,n):
        ...
        source = supernode['contains']
        for _ in range(n):
            next_hop_neighbors = set()
            for oid in source:
                neighbors = set(self.g.neighbors(oid))
                next_hop_neighbors.add(neighbors)
            source = next_hop_neighbors
        supernodes = set([self.oid_to_sname[i] for i in source])
        return supernodes
    ...
    def make_drawable(self):
        ...
        for e in self.s.es:
            ...
            e['width'] = 5 * self.get_support_of_edge_between_supernodes(source_name, target_name)
            e['label'] = self.get_support_of_edge_between_supernodes(source_name, target_name)

    def summarize(self):
        unvisited = set([self.get_supernode_name(i) for i in range(0,self.s.vcount)])
        ...
        while len(unvisited) > 0:
            ...
            to_merge.add(u)
            self.merge_supernodes(to_merge)
            self.update_unvisited(unvisited,to_merge)
